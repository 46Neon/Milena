...
int main(void) {
    ...
    const char *spill_directory = "tests/.stream_spill_tmp";
    if (mkdir(spill_directory, 0700) != 0) assert(errno == EEXIST);
    assert(chmod(spill_directory, 0700) == 0);
    ...
    grouped_options.max_resident_groups = 1;
    grouped_options.spill_directory = spill_directory;
    ...
    /* A header-only budget fails closed and preserves an existing destination. */
    json = fopen(group_output, "wb");
    assert(json != NULL);
    assert(fputs("sentinel", json) >= 0);
    assert(fclose(json) == 0);
    ...
    assert(json != NULL);
    memset(buffer, 0, sizeof(buffer));
    assert(fread(buffer, 1, sizeof(buffer) - 1, json) == strlen("sentinel"));
    assert(fclose(json) == 0);
    assert(strcmp(buffer, "sentinel") == 0);
    /* The failed operation must release named runs before the next call. */
    grouped_options.max_spill_bytes = 0;
    assert(remove(group_output) == 0);
    milena_error_clear(&grouped_error);
    assert(milena_stream_csv_grouped_with_options(group_input, group_output,
        "zona", grouped_metrics, 2, &grouped_options, &grouped_report,
        &grouped_error) == MILENA_OK);
    ...
    assert(fputs("zona,importe,referencia\n", empty_group_file) >= 0);
    ...
    remove(group_output);
    assert(rmdir(spill_directory) == 0);
