...
static void build_phi_diamond(IRProgram *program, bool wrong_edge_value) {
    assert(ir_program_add_block(program, 1));
    assert(append(program, 1, IR_CONST_I64, 1, IR_TYPE_I64, 0, 0, 5, 0, 0));
    assert(append(program, 1, IR_CONST_I64, 2, IR_TYPE_I64, 0, 0, 5, 0, 0));
    assert(append(program, 1, IR_EQ_I64, 3, IR_TYPE_BOOL, 1, 2, 0, 0, 0));
    assert(append(program, 1, IR_COND_BRANCH, 0, IR_TYPE_VOID, 3, 0, 0, 10, 20));

    assert(ir_program_add_block(program, 10));
    assert(append(program, 10, IR_CONST_I64, 4, IR_TYPE_I64, 0, 0, 40, 0, 0));
    assert(append(program, 10, IR_BRANCH, 0, IR_TYPE_VOID, 0, 0, 0, 30, 0));
    ...
    assert(ir_program_add_block(program, 30));
    assert(ir_program_add_block_parameter(program, 30, 6, IR_TYPE_I64));
    assert(append(program, 30, IR_ADD_I64, 7, IR_TYPE_I64, 6, 1, 0, 0, 0));
    assert(append(program, 30, IR_RETURN, 0, IR_TYPE_I64, 7, 0, 0, 0, 0));
    assert(ir_block_add_edge_argument(program, 10, 30, 0, 4));
    assert(ir_block_add_edge_argument(program, 20, 30, 0, 5));
}
...
static void test_rejects_undominated_direct_use_and_unreachable_block(void) {
    ...
    assert(ir_program_add_block(program, 1));
    assert(append(program, 1, IR_RETURN, 0, IR_TYPE_VOID, 0, 0, 0, 0, 0));
    assert(ir_program_add_block(program, 2));
    assert(append(program, 2, IR_RETURN, 0, IR_TYPE_VOID, 0, 0, 0, 0, 0));
    assert(!ir_program_validate(program, error, sizeof(error)));
