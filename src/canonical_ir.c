...
bool milena_ir_program_set_function_signature(MilenaIRProgram *program,
    ...
    if (!program || program->has_function_signature ||
        program->instructions || program->call_arguments || program->blocks ||
        program->parameters || program->edge_arguments || program->count ||
        program->call_argument_count || program->call_argument_capacity ||
        program->block_count ||
...
static bool ir_reserve_call_arguments(MilenaIRProgram *program, size_t required) {
    uint32_t *grown;
    size_t capacity;
    if (required <= program->call_argument_capacity) return true;
    capacity = program->call_argument_capacity == 0 ? 8 :
               program->call_argument_capacity;
    while (capacity < required) {
        if (capacity > SIZE_MAX / 2) { capacity = required; break; }
        capacity *= 2;
    }
    if (capacity < required || capacity > SIZE_MAX / sizeof(*grown)) return false;
    grown = realloc(program->call_arguments, capacity * sizeof(*grown));
    if (!grown) return false;
    program->call_arguments = grown;
    program->call_argument_capacity = capacity;
    return true;
}
...
bool milena_ir_block_append_call(MilenaIRProgram *program, uint32_t block_id,
    ...
    size_t start;
    size_t required;
    if (!program || !target_symbol_id || (argument_count && !argument_ids) ||
        argument_count > SIZE_MAX / sizeof(*copy) ||
        program->call_argument_count > SIZE_MAX - argument_count ||
        (program->call_argument_count && !program->call_arguments) ||
        program->call_argument_capacity < program->call_argument_count ||
        program->call_argument_capacity > SIZE_MAX / sizeof(*program->call_arguments))
        return false;
    if (argument_count) {
        copy = malloc(argument_count * sizeof(*copy));
        if (!copy) return false;
        memcpy(copy, argument_ids, argument_count * sizeof(*copy));
    }
    start = program->call_argument_count;
    required = start + argument_count;
    if (!ir_reserve_call_arguments(program, required)) {
        free(copy);
        return false;
    }
    if (!milena_ir_block_append_instruction(program, block_id, MILENA_IR_CALL,
            result_id, result_type, 0, 0, (int64_t)target_symbol_id,
            0.0, 0, 0)) {
        free(copy);
        return false;
    }
    if (argument_count)
        memcpy(program->call_arguments + start, copy,
               argument_count * sizeof(*copy));
    free(copy);
    program->call_argument_count = required;
    MilenaIRInstruction *instruction = &program->instructions[program->count - 1];
    instruction->call_argument_offset = start;
    instruction->call_argument_count = argument_count;
    return true;
}
...
bool milena_ir_program_validate(const MilenaIRProgram *program, char *error,
    ...
    if (!program || !program->instructions || !program->blocks ||
        ...
        (program->call_argument_capacity && !program->call_arguments) ||
        (!program->call_argument_capacity && program->call_arguments) ||
        program->call_argument_count > program->call_argument_capacity ||
        program->call_argument_capacity > SIZE_MAX / sizeof(*program->call_arguments))
        return ir_fail(error, error_capacity,
            ...
            switch (ins->opcode) {
                ...
                case MILENA_IR_CALL:
                    ...
                    if (!program->module_context || !ins->integer_immediate ||
                        ins->integer_immediate > UINT32_MAX || ins->operand1_id ||
                        ins->operand2_id || ins->target_true || ins->target_false ||
                        ins->float_immediate != 0.0 ||
        ...
        for (size_t ii = block->first_instruction; ii < end; ++ii) {
            ...
            switch (ins->opcode) {
                ...
                case MILENA_IR_CALL: {
                    ...
                    for (size_t ai = 0; ai < callee->parameter_count; ++ai) {
                        uint32_t argument_id = program->call_arguments[
                            ins->call_argument_offset + ai];
                        if (!ir_use_is_valid(values, value_count, argument_id,
                                callee->parameter_types[ai], bi, ii, n,
                                dominators, error, error_capacity)) {
                            free(values); free(reachable); free(queue); free(dominators);
                            return false;
                        }
                    }
...
static bool ir_scalar_lower_expression(MilenaIRProgram *program, uint32_t block_id,
    ...
    switch (expression->kind) {
        ...
        case MILENA_HIR_EXPR_CALL: {
            ...
            const MilenaIRModuleFunction *callee;
            uint32_t *argument_ids = NULL;
            size_t argument_count = expression->as.call.argument_count;
            MilenaIRType expected_return;
            if (!module || !expression->resolved_symbol_id ||
                expression->resolved_symbol_id > UINT32_MAX)
                return ir_fail(error, error_capacity,
                               "unresolved function call in canonical IR module");
            callee = ir_module_find_function(module,
                         (uint32_t)expression->resolved_symbol_id);
            if (!callee)
                return ir_fail(error, error_capacity,
                               "unresolved function call in canonical IR module");
            if (argument_count != callee->parameter_count)
                return ir_fail(error, error_capacity,
                               "function call arity disagrees with the resolved signature");
            if (argument_count && (!expression->as.call.arguments ||
                !callee->parameter_types ||
                argument_count > SIZE_MAX / sizeof(*argument_ids)))
                return ir_fail(error, error_capacity,
                               "function call arguments or signature are missing");
            ...
            if (argument_count) {
                argument_ids = malloc(argument_count * sizeof(*argument_ids));
                if (!argument_ids)
                    return ir_fail(error, error_capacity,
                                   "out of memory lowering function call arguments");
            }
            for (size_t i = 0; i < argument_count; ++i) {
                ...
                if (actual_type != callee->parameter_types[i]) {
                    free(argument_ids);
                    return ir_fail(error, error_capacity,
            ...
            if (*next_value == UINT32_MAX ||
                !milena_ir_block_append_call(program, block_id, *next_value,
                    expected_return, callee->symbol_id, argument_ids,
                    argument_count)) {
                free(argument_ids);
                return ir_fail(error, error_capacity,
...
static bool ir_scalar_lower_assignment_sequence(
    ...
    IRScalarBinding *bindings, size_t *binding_count, size_t binding_capacity,
    unsigned depth, char *error, size_t error_capacity) {
    if (!binding_count || *binding_count > binding_capacity)
        return ir_fail(error, error_capacity,
                       "typed scalar binding environment is invalid");
    if (depth > 128)
        return ir_fail(error, error_capacity,
    ...
    for (size_t i = 0; i < statement_count; ++i) {
        ...
        if (statement->kind == MILENA_HIR_STMT_ASSIGN ||
            statement->kind == MILENA_HIR_STMT_DECLARE) {
            size_t index;
            uint32_t value_id;
            ...
            if (!statement->resolved_symbol_id || !statement->as.expression ||
                ...
                return ir_fail(error, error_capacity,
                    "typed scalar si/sino declaration or assignment lacks resolved typed data");
            index = ir_scalar_binding_index(bindings, *binding_count,
                                            statement->resolved_symbol_id);
            if (statement->kind == MILENA_HIR_STMT_ASSIGN &&
                (index == SIZE_MAX || bindings[index].type != statement_type))
                return ir_fail(error, error_capacity,
                    "typed scalar conditional assignment has no compatible binding in scope");
            if (statement->kind == MILENA_HIR_STMT_DECLARE &&
                (index != SIZE_MAX || *binding_count >= binding_capacity))
                return ir_fail(error, error_capacity,
                    "typed scalar conditional declaration duplicates or exceeds its binding scope");
            ...
            if (expression_type != statement_type ||
                (statement->kind == MILENA_HIR_STMT_ASSIGN &&
                 bindings[index].type != statement_type))
                return ir_fail(error, error_capacity,
                    "typed scalar conditional declaration or assignment has an incompatible type");
            if (statement->kind == MILENA_HIR_STMT_DECLARE) {
        ...
        if (statement->kind == MILENA_HIR_STMT_IF) {
            ...
            *next_block_id += 3;
            ...
            if (binding_capacity > SIZE_MAX / sizeof(*then_bindings))
                return ir_fail(error, error_capacity,
                               "too many scalar bindings in conditional");
            if (binding_capacity) {
                then_bindings = malloc(binding_capacity * sizeof(*then_bindings));
                else_bindings = malloc(binding_capacity * sizeof(*else_bindings));
                if (!then_bindings || !else_bindings) {
                    free(then_bindings); free(else_bindings);
                    return ir_fail(error, error_capacity,
                        "out of memory lowering nested scalar conditional scope");
                }
                if (outer_binding_count) {
                    memcpy(then_bindings, bindings,
                           outer_binding_count * sizeof(*then_bindings));
                    memcpy(else_bindings, bindings,
                           outer_binding_count * sizeof(*else_bindings));
                }
            }
            if (!ir_scalar_lower_expression(program, *current_block_id,
                    statement->as.conditional.condition, bindings, outer_binding_count,
            ...
            if (!ir_scalar_lower_assignment_sequence(program,
                    ...
                    &then_binding_count, binding_capacity, depth + 1,
                    error, error_capacity) ||
            ...
            if (!ir_scalar_lower_assignment_sequence(program,
                    ...
                    &else_binding_count, binding_capacity, depth + 1,
                    error, error_capacity) ||
            ...
            for (size_t binding_index = 0;
                 binding_index < outer_binding_count; ++binding_index) {
        ...
        return ir_fail(error, error_capacity,
            "typed scalar conditional arms support declarations, assignments and nested si/sino only; returns and other statements are unsupported");
    }
    return true;
}
...
static bool ir_scalar_count_statement_tree(
    const MilenaHIRStatement *const *statements, size_t statement_count,
    unsigned depth, size_t *total) {
    if ((statement_count && !statements) || !total || depth > 128) return false;
    for (size_t i = 0; i < statement_count; ++i) {
        const MilenaHIRStatement *statement = statements[i];
        if (!statement || *total == SIZE_MAX) return false;
        ++*total;
        if (statement->kind == MILENA_HIR_STMT_IF) {
            if ((statement->as.conditional.then_count &&
                 !statement->as.conditional.then_body) ||
                (statement->as.conditional.else_count &&
                 !statement->as.conditional.else_body) ||
                !ir_scalar_count_statement_tree(
                    (const MilenaHIRStatement *const *)
                        statement->as.conditional.then_body,
                    statement->as.conditional.then_count, depth + 1, total) ||
                !ir_scalar_count_statement_tree(
                    (const MilenaHIRStatement *const *)
                        statement->as.conditional.else_body,
                    statement->as.conditional.else_count, depth + 1, total))
                return false;
        }
    }
    return true;
...
static bool ir_program_lower_scalar_function_body_context(
    ...
    size_t binding_capacity, statement_capacity = 0;
    ...
    if (!program || !function || !function->name || !function->resolved_symbol_id ||
        ...
        program->call_arguments || program->call_argument_count ||
        program->call_argument_capacity || program->block_count ||
        program->block_capacity || program->parameter_count ||
    ...
    if (!function->parameters && function->parameter_count) return ir_fail(
        error, error_capacity, "scalar HIR parameter list is missing");
    if (function->parameter_count >= UINT32_MAX ||
        !ir_scalar_count_statement_tree(
            (const MilenaHIRStatement *const *)function->body,
            function->body_count, 0, &statement_capacity) ||
        function->parameter_count > SIZE_MAX - statement_capacity)
        return ir_fail(error, error_capacity,
                       "scalar HIR statement tree is malformed or too large");
    binding_capacity = statement_capacity + function->parameter_count;
        ...
        if (statement->kind == MILENA_HIR_STMT_IF) {
            ...
            if (binding_capacity) {
                ...
                if (!then_bindings || !else_bindings) {
                    free(then_bindings); free(else_bindings);
                    ir_fail(error, error_capacity,
                            "out of memory lowering scalar conditional scope");
                    goto cleanup;
                }
                if (outer_binding_count) {
                    memcpy(then_bindings, bindings,
            ...
            if (!ir_scalar_lower_assignment_sequence(lowered,
                    ...
                    (const MilenaHIRStatement *const *)statement->as.conditional.then_body,
                    statement->as.conditional.then_count, then_bindings,
                    &then_binding_count, binding_capacity, 0,
                    error, error_capacity) ||
            ...
            if (!ir_scalar_lower_assignment_sequence(lowered,
                    ...
                    (const MilenaHIRStatement *const *)statement->as.conditional.else_body,
                    statement->as.conditional.else_count, else_bindings,
                    &else_binding_count, binding_capacity, 0,
                    error, error_capacity) ||
...
bool milena_ir_module_validate(const MilenaIRModule *module, char *error,
    ...
    for (size_t i = 0; i < module->function_count; ++i) {
        ...
        if (!function->name || !function->symbol_id || !function->body ||
            (function->parameter_count &&
             (!function->parameter_types ||
              !function->body->signature.parameter_types)) ||
            function->parameter_count > SIZE_MAX / sizeof(*function->parameter_types) ||
        ...
        for (size_t pi = 0; pi < function->parameter_count; ++pi)
            if (function->parameter_types[pi] !=
                function->body->signature.parameter_types[pi]) {
                free(state);
                return ir_fail(error, error_capacity,
                               "module and body parameter signatures disagree");
            }
...
bool milena_ir_module_lower_scalar_hir(MilenaIRModule **output,
    ...
    for (size_t i = 0; i < hir->function_count; ++i) {
        ...
        if (!source->name || !source->resolved_symbol_id ||
            ...
            source->parameter_count >= UINT32_MAX ||
            (source->parameter_count && !source->parameters) ||
            source->parameter_count > SIZE_MAX / sizeof(*target->parameter_types)) {
            ir_fail(error, error_capacity,
                    "function identity or parameter list is invalid or too large");
            goto fail;
        }
