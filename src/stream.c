...
static bool stream_spill_directory_private(const char *directory) {
#if defined(_WIN32)
    (void)directory;
    return false;
#else
    struct stat info;
    if (!directory || !directory[0] || lstat(directory, &info) != 0 ||
        !S_ISDIR(info.st_mode) || info.st_uid != geteuid() ||
        (info.st_mode & (S_IRWXG | S_IRWXO)) != 0) return false;
    return true;
#endif
}

static FILE *stream_spill_create_file(const char *directory, char **path_out) {
#if defined(_WIN32)
    (void)directory; (void)path_out;
    return NULL;
#else
    static const char suffix[] = "/.milena-spill-XXXXXX";
    if (!stream_spill_directory_private(directory) || !path_out) return NULL;
    size_t length = strlen(directory);
    bool slash = length > 0 && directory[length - 1] == '/';
    size_t suffix_length = sizeof(suffix) - (slash ? 1u : 0u);
    if (length > SIZE_MAX - suffix_length) return NULL;
    char *path = (char *)malloc(length + suffix_length);
    if (!path) return NULL;
    if (slash) snprintf(path, length + suffix_length, "%s%s", directory, suffix + 1);
    else snprintf(path, length + suffix_length, "%s%s", directory, suffix);
    int fd = mkstemp(path);
    if (fd < 0) { free(path); return NULL; }
    (void)fchmod(fd, S_IRUSR | S_IWUSR);
    FILE *file = fdopen(fd, "w+b");
    if (!file) { close(fd); (void)unlink(path); free(path); return NULL; }
    *path_out = path;
    return file;
#endif
}

static FILE *stream_output_temp_create(const char *output_path, char **path_out) {
#if defined(_WIN32)
    (void)output_path; (void)path_out;
    return NULL;
#else
    static const char suffix[] = ".milena-output-XXXXXX";
    if (!output_path || !path_out) return NULL;
...
static void stream_spill_discard(FILE *file, char *path) {
...
static MilenaStatus stream_spill_write_run(StreamGroup *groups, size_t group_count,
        ...
        const char *spill_directory, size_t *spill_bytes,
        size_t *spill_records, MilenaError *error) {
    ...
    char *path = NULL;
    FILE *file = stream_spill_create_file(spill_directory, &path);
    if (!file) {
        milena_error_set(error, MILENA_ERR_IO, 0, 0, 0,
                         "No se pudo crear una corrida privada en el directorio temporal configurado");
        return MILENA_ERR_IO;
    }
    ...
    *spill_records += group_count;
    return MILENA_OK;
limit_or_io:
    stream_spill_discard(file, path);
...
MilenaStatus milena_stream_csv_grouped_with_options(
    ...
    size_t resident_group_limit = options->max_resident_groups
        ? options->max_resident_groups :
          (options->spill_enabled && group_limit > STREAM_DEFAULT_MAX_GROUPS
              ? STREAM_DEFAULT_MAX_GROUPS : group_limit);
    if (resident_group_limit > group_limit) resident_group_limit = group_limit;
    ...
    if (!input_path || !output_path || !group_column || !group_column[0] ||
        ...
        options->max_resident_groups > STREAM_HARD_MAX_GROUPS ||
        (!options->spill_enabled && options->max_resident_groups != 0) ||
        ...
        (options->spill_enabled && (!options->spill_directory ||
         spill_byte_limit < STREAM_SPILL_HEADER_BYTES ||
    ...
    if (options->spill_enabled &&
        !stream_spill_directory_private(options->spill_directory)) {
        milena_error_set(error, MILENA_ERR_IO, 0, 0, 0,
            "El directorio temporal debe existir, pertenecer al usuario y no conceder permisos a grupo u otros");
        return MILENA_ERR_IO;
    }
    ...
    if (resident_group_limit > SIZE_MAX / sizeof(*groups)) {
        milena_error_set(error, MILENA_ERR_OVERFLOW, 0, 0, 0,
                         "El límite de grupos desborda el estado de flujo");
        status = MILENA_ERR_OVERFLOW;
        goto grouped_finish;
    }
    size_t group_array_bytes = resident_group_limit * sizeof(*groups);
    if (resident_group_limit > SIZE_MAX / 2) {
        milena_error_set(error, MILENA_ERR_OVERFLOW, 0, 0, 0,
    ...
    size_t desired_slots = resident_group_limit * 2;
    ...
    groups = (StreamGroup *)calloc(resident_group_limit, sizeof(*groups));
    ...
    while ((status = stream_read_record(input, &record, &record_capacity,
        ...
        if (group_index_found == group_count) {
            ...
            if (group_count >= resident_group_limit) {
                if (!options->spill_enabled) {
                    milena_error_set(error, MILENA_ERR_OVERFLOW, 0, 0, 0,
                ...
                status = stream_spill_write_run(groups, group_count, metric_count,
                    ...
                    spill_byte_limit, spill_record_limit, options->spill_directory,
                    &spill_bytes, &spill_records, error);
    ...
    if (input && fclose(input) != 0) {
        input = NULL;
        milena_error_set(error, MILENA_ERR_IO, 0, 0, 0,
                         "No se pudo cerrar el CSV agrupado después de leerlo");
        status = MILENA_ERR_IO;
        goto grouped_finish;
    }
    input = NULL;
    ...
    if (spilled && group_count > 0) {
        status = stream_spill_write_run(groups, group_count, metric_count,
            ...
            spill_record_limit, options->spill_directory, &spill_bytes,
            &spill_records, error);
    ...
    if (!spilled) {
        ...
        if (emitted && total_group_count > group_limit) {
            milena_error_set(error, MILENA_ERR_OVERFLOW, 0, 0, 0,
                "La agrupación superó el límite total de grupos configurado");
            resource_limit_reached = true;
            status = MILENA_ERR_OVERFLOW;
            emitted = false;
        }
        ...
        if (emitted) {
            output = stream_output_temp_create(output_path, &output_temp_path);
            if (!output) {
                milena_error_set(error, MILENA_ERR_IO, 0, 0, 0,
                                 "No se pudo crear el reporte temporal para publicación atómica");
                status = MILENA_ERR_IO;
                emitted = false;
            }
        }
        if (emitted) output_created = true;
    ...
    if (output) {
        if (fflush(output) != 0) emitted = false;
#if !defined(_WIN32)
        if (spilled && fsync(fileno(output)) != 0) emitted = false;
#endif
        if (fclose(output) != 0) emitted = false;
        output = NULL;
    }
    if (emitted && spilled && output_temp_path) {
        if (rename(output_temp_path, output_path) != 0) {
            emitted = false;
            milena_error_set(error, MILENA_ERR_IO, 0, 0, 0,
                             "No se pudo publicar atómicamente el reporte agrupado");
            status = MILENA_ERR_IO;
        } else {
    ...
    if (!emitted) {
        ...
        if (output_created && !spilled) remove(output_path);
        if (output_temp_path) { (void)remove(output_temp_path); free(output_temp_path); output_temp_path = NULL; }
        goto grouped_finish;
    }
...
#if !defined(_WIN32)
            if (spill_runs[r].path) (void)unlink(spill_runs[r].path);
#else
            if (spill_runs[r].path) (void)remove(spill_runs[r].path);
#endif
            free(spill_runs[r].path);
        }
        free(spill_runs);
    }
    free(record);
