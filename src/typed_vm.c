...
static bool vm_value_type_matches(MilenaIRType type,
                                  const MilenaTypedVMValue *value) {
...
static bool vm_transfer_block(VMExecution *execution,
    ...
    MilenaTypedVMValue *incoming = NULL;
    for (size_t i = 0; i < program->parameter_count; ++i)
        if (program->parameters[i].block_id == target_block) ++parameter_count;
    if (parameter_count) {
        incoming = (MilenaTypedVMValue *)vm_calloc(execution, parameter_count,
                                                    sizeof(*incoming));
        if (!incoming)
...
static bool vm_execute_function(VMExecution *execution,
    ...
    for (;;) {
        ...
        for (size_t ii = block->first_instruction; ii < end; ++ii) {
            ...
            MilenaTypedVMValue left = {0}, right = {0}, produced = {0};
            uint32_t target = 0;
            ...
            switch (ins->opcode) {
                ...
                case MILENA_IR_CALL: {
                    ...
                    MilenaTypedVMValue *call_arguments = NULL;
                    MilenaTypedVMValue call_result = {0};
                    ...
                    if (ins->call_argument_count) {
                        call_arguments = (MilenaTypedVMValue *)vm_calloc(
