...
The canonical program now owns a typed Arrow HIR descriptor copied from the validated logical/physical plan: source/output paths, typed projections, optional predicate, resource limits, and available source spans. The runtime consumes this owned descriptor; it does not borrow plan fields from the AST. The strict compiler-input boundary still rejects this HIR because lowering to the portable typed IR/MLBC and reference VM is not implemented. This does not change the WIP status above.
...
Null predicate values do not match; projected nulls retain their validity. Root stream-schema metadata and metadata on projected fields are copied to the output schema; unprojected fields and their metadata are omitted. `int64` values are copied as signed 64-bit integers (not via `double`), preserving values above 2^53. The numeric filter literal is a Milena binary64 number; for `int64` fields the comparison is performed using `floor(literal)` to avoid converting each data value to binary64. For standalone execution, all input string values remain subject to strict UTF-8 validation. Only when a compatible text-equality plan is derived from the shared Arrow/typed-SQL logical descriptor, the backend first filters bytewise against a non-NULL valid UTF-8 operand, then validates UTF-8 only in projected text values of rows returned. This matches typed SQLite result validation without relaxing existing standalone Arrow programs; SQL raw is outside the shared contract.
