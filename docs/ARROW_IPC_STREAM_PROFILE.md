...
The canonical program now owns the validated Arrow logical/physical plan and exposes a borrowed view only through the compatibility `MilenaCanonicalCompilerInput`; the strict HIR/compiler entry continues to reject Arrow until a real lowering exists. This is plan ownership, not typed HIR, lowering to the portable typed IR/MLBC, or execution by the reference VM; it does not change the WIP status above. The plan's AST references are valid only while the owning canonical program remains alive.
