...
`make test-data-engine-parity` executes real `.milena` programs over one synthetic CSV on both the typed in-memory table and CSV streaming paths. It compares grouped `suma`/`media`/`conteo` after a numeric `>` filter, separately checks numeric `conteo` parity for empty and nonnumeric cells without a filter, and verifies a row-budget failure preserves an existing output file. `test_query_plan` verifies that the semantically equivalent HIR and CSV plans normalize to the same logical graph while retaining distinct materialized versus record-streaming execution modes, and that an unsupported aggregate is rejected from the common subset. The grouped-spill runtime test separately compares the stream in-memory grouping with a forced spill request and checks scratch cleanup. `make test` includes these gates.
...
The contract is deliberately limited to one declared text key, an optional strict numeric `>` predicate applied before grouping, and 1–16 numeric input metrics using `SUM`, `MEAN` or `COUNT`. Numeric count means the number of valid numeric cells (not merely non-empty cells); the streaming adapter opts into that behavior while preserving legacy non-empty counting for untyped/text stream metrics. The end-to-end parity fixture includes empty and nonnumeric numeric cells to exercise that distinction. The adapters retain their own physical plans, resource policies, and execution: the materialized adapter loads a table, whereas the CSV adapter scans complete records and may use its documented spill plan. The runtime uses the normalized common plan for the stream's source, filter, metrics and sink, and validates the HIR overlap/sink before execution.
