...
1. **WinGet:** el manifiesto versionado debe validarse y aceptarse en una fuente consultable por WinGet; adjuntarlo a GitHub Releases no lo indexa automáticamente. Microsoft documenta el proceso de [validación y envío del manifiesto](https://learn.microsoft.com/en-us/windows/package-manager/package/repository) y recibe contribuciones en [`microsoft/winget-pkgs`](https://github.com/microsoft/winget-pkgs). La aceptación depende de sus validaciones/revisión.
2. **APT Linux:** debe publicarse un repositorio Linux firmado (o incorporarse el paquete a un archivo oficial), con índice y artefactos por arquitectura. Si se usa un repositorio propio, registrar su clave y fuente también es un paso de bootstrap; `apt install milena` por sí solo no configura una fuente desconocida. Seguir la guía de [repositorios de terceros de Debian](https://wiki.debian.org/DebianRepository/UseThirdParty) o [Ubuntu](https://ubuntu.com/server/docs/explanation/software/third-party-repository-usage/).
3. **Termux:** para que `pkg install milena` funcione en Termux sin pasos previos, la receta debe ser aceptada y publicada por [`termux/termux-packages`](https://github.com/termux/termux-packages). La contribución debe seguir su [guía oficial](https://github.com/termux/termux-packages/blob/master/CONTRIBUTING.md). Un repositorio APT propio requiere registrar primero su fuente y clave; no equivale a estar en el repositorio oficial de Termux.
...
- Construir paquetes Linux por arquitectura declarada; verificar control, dependencias, rutas, checksum y procedencia. Sustituir el correo de Maintainer de ejemplo por un contacto real aprobado.
