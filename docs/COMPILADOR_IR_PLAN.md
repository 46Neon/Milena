...
Las pruebas de frontend cubren la ruta canónica, anotaciones numéricas, operandos estructurados, rechazo de una operación de tipos incompatibles con posición y limpieza del AST en error; también ejecutan pruebas del intérprete para detectar regresiones. Esta es una base de tipado, **no una HIR** y tampoco una migración del conjunto completo de operaciones/datos a nodos estructurados. Siguen pendientes: modelo tipado por constructo para todo el lenguaje canónico (incluyendo data operations y contratos de operandos), resolución y bindings semánticos de nombres/ámbitos/funciones, HIR explícita poseída por el programa canónico, y pruebas e2e de equivalencia/errores/cleanup de esa HIR. Por tanto la fase 2 sigue abierta y no se debe iniciar fase 3.
