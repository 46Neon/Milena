...
The host loader reads CSV with comma delimiter and the canonical `Dataset`/typed-table conversion path. It must enforce the encoded and hard limits while reading, not merely after unbounded allocation: a 64 MiB total input-file budget, 5,000 data rows, 70 columns, and 1 MiB per field. `dataset_load_csv_with_limits` already accepts row, column and field caps; the total-byte cap therefore needs an enforced bounded-reader extension (including record/header bytes), not a `stat()`-only preflight susceptible to file changes during reading. No network/cloud source, stream mode, second source, or alternate reader is represented.
