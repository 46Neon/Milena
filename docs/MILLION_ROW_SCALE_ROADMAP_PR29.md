...
| Candidato de resolución inicial | Commit `53b8e78c9d8a9b5efa72d37bef657df0a5145940` y corrección del test en `86a68c7c87454dae40935f20df8c6431c8eca068`, sobre la base actual. Se consolidaron planner/runtime canónicos y salvaguardas de CSV; no hay compilador C ni `make` local. | Ninguno de estos SHA tiene validación completa: la primera corrida detectó una llamada de prueba incompleta, ya corregida en el head siguiente. |
| CI del head `86a68c7` | Éxitos: GCC/Clang, ASan/UBSan, paquete Debian, Windows y la agregación/paridad de 1.000.000 de filas. El gate de 10 GB de Windows aprobó; el gate Linux falló porque el reporte en memoria omitía `bytes_leidos`. | Las correcciones de límites del join y de telemetría están en el head posterior `905d8c4`; ese SHA tuvo los 12 checks de PR en éxito, incluidos gates 10 GB Linux y Windows. El head actual debe validar de nuevo al incluir el plan. |
| Publicador Termux APT manual | La base `main` tenía YAML inválido en `.github/workflows/publish-apt.yml:76`: el cuerpo de un heredoc Python quedaba fuera de `run: |`. Se corrigió la indentación en esta rama; el workflow requiere confirmación manual y no fue ejecutado. | Cambio de workflow pendiente de validación YAML/CI en el head actual; no se publicó ningún paquete. |
