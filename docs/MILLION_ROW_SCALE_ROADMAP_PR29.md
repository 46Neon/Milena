...
| CI del head `86a68c7` | Éxitos: Windows C17, Windows x64 build, Markdown, Cloudflare y el E2E de agregación CSV global de 1.000.000 de filas. Fallos: suites C/Linux y paquete Debian por la prueba heredada de `join` que esperaba hacer cumplir su límite AST; los gates Linux/Windows de 10 GB no pudieron verificar `bytes_leidos` en el reporte de agrupación en memoria. | Causas localizadas; correcciones al join acotado y al campo de telemetría preparadas para el siguiente head. No se afirma aprobado el gate de 10 GB. |
