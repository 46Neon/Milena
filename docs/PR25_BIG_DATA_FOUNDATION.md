...
La agrupación conserva acumuladores en memoria por omisión y falla al alcanzar sus presupuestos. Un corte opt-in ya conecta la agrupación tipada del CSV streaming con el reducer spillable sin materializar filas ni grupos; la operación tabular sobre Dataset/MilenaTable sigue materializando en RAM. La rama `main` tiene un planificador/ejecutor local genérico que describe rangos contiguos de bytes y ejecuta trabajadores locales, pero sus rangos no reconocen límites de registros ni comillas CSV; no se usa para leer ni derramar estos agregados. El ejecutor no equivale a un motor distribuido.

El spill del CSV streaming reutiliza `src/stream.c`, `src/grouped_aggregate.c` y `src/spill_store.c` como fuentes de producto ya clasificadas; no agrega un parser/backend CSV paralelo. El manifiesto sigue clasificando cada fuente C presente y no oculta módulos futuros o experimentales.

El estado y los límites del corte están en [`GROUPED_SPILL_CONTRACT.md`](GROUPED_SPILL_CONTRACT.md): una clave texto/una métrica, límites independientes de búfer CSV y reducer, staging transaccional y pruebas de equivalencia/errores. No se afirma RSS total medido ni validación industrial. Arrow/Parquet, nube, Spark, Flink, red y ejecución distribuida tampoco forman parte de esta fase; no se simulan ni se prometen.
