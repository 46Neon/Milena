...
El siguiente incremento debe diseñar y probar límites de registros CSV (incluidos saltos de línea entre comillas), particiones deterministas, un único contrato tipado de spill con versión y checksum, límites de disco/archivos y fusión con memoria acotada. Hasta completar equivalencia, fallos/limpieza de temporales y benchmarks reproducibles, PR25 no afirma que spill esté implementado. Arrow/Parquet, nube, Spark, Flink, red y ejecución distribuida tampoco forman parte de esta fase; no se simulan ni se prometen.
