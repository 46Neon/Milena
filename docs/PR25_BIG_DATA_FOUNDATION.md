...
La agrupación conserva sus acumuladores en memoria y, por diseño, falla al alcanzar sus presupuestos: **no implementa spill-to-disk ni particionado de CSV**. La rama `main` tiene un planificador/ejecutor local genérico que describe rangos contiguos de bytes y ejecuta trabajadores locales, pero sus rangos no reconocen límites de registros ni comillas CSV; no se usa para leer, derramar o combinar estos agregados. El ejecutor no equivale a un motor distribuido.

En el head de PR25 inspeccionado, `src/spill.c` y `src/spill_store.c` no existen; `src/stream.c` es el backend de producto agrupado y sigue usando la tabla de grupos acotada en RAM. El manifiesto clasifica todas las 51 fuentes C presentes (32 producto, 5 lenguaje, 3 funciones, 11 experimentales); no hay fuentes sin categoría en este head. Los nombres de módulos futuros no deben añadirse como si existieran ni ocultarse del manifiesto.

El contrato para el siguiente incremento está en [`GROUPED_SPILL_CONTRACT.md`](GROUPED_SPILL_CONTRACT.md): define el formato tipado versionado, checksums, budgets, merge estable y fallos transaccionales, además de los criterios de aceptación. Hace falta diseñar y probar límites de registros CSV (incluidos saltos de línea entre comillas), particiones deterministas, límites de disco/archivos y fusión con memoria acotada. Hasta completar equivalencia, fallos/limpieza de temporales y benchmarks reproducibles, PR25 no afirma que spill esté implementado. Arrow/Parquet, nube, Spark, Flink, red y ejecución distribuida tampoco forman parte de esta fase; no se simulan ni se prometen.
