...
El workflow añade un gate separado con un CSV generado de al menos
10.000.000.000 bytes, 1.000.000 de filas y exactamente 1.000 grupos. El límite de
262.144 bytes de memoria del reducer hace que la cardinalidad del fixture exceda
su mapa residente; el gate exige un modo `flujo_agrupado_spill`, scratch de disco
muestreado no nulo y paridad exacta con la ruta en memoria, además del consumo
exacto de bytes por ambas rutas. Registra tamaño lógico y asignado del CSV,
filas, grupos, RSS de proceso y bytes de scratch en disco (no RAM). Linux y
Windows x64 ejecutan el gate con el flujo de posición de archivos de 64 bits; la
prueba Windows depende de que el runner tenga espacio libre suficiente y debe
confirmarse con la CI del nuevo head. Este fixture no demuestra procesamiento
arbitrario de alta cardinalidad, una cota global de RAM, Termux/AArch64 ni
procesamiento distribuido. No se marcará como validado hasta que finalice con
éxito la CI del head que incorpora el gate.
