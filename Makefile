...
.PHONY: test-stream-format

test-stream-format: tests/test_stream_format
	./tests/test_stream_format

.PHONY: check-grouped-spill

check-grouped-spill:
	python3 scripts/check_grouped_spill.py

tests/test_stream_format: tests/test_stream_format.c src/stream.c src/common.c
	$(CC) $(CFLAGS) tests/test_stream_format.c src/common.c $(LDFLAGS) -o $@
...
test: check-source-manifest check-experimental-isolation check-stream-architecture check-grouped-spill check-unification-architecture check-termux-packaging check-termux-runner-contract check-compiler-boundary test-termux-packaging test-canonical-compiler benchmark-stream benchmark-stream-grouped $(TARGET) test-sst test-array test-array-worker2 test-array-worker3 test-forest test-arena test-table test-table-worker4 test-pr21-regressions test-finance test-stream test-stream-format test-partition-plan test-partition-executor test-partition-equivalence test-partition-concurrency test-partition-reduce test-partition-budget test-process-executor test-entrypoints \
...
clean:
	rm -f $(OBJECTS) $(FUNCTION_OBJECTS) $(TARGET) tests/test_sst_modules \
		...
		tests/test_finance tests/test_pr21_regressions tests/test_stream tests/test_stream_format tests/test_partition_plan tests/test_partition_executor tests/test_partition_equivalence tests/test_partition_concurrency tests/test_partition_reduce tests/test_partition_budget tests/test_process_executor tests/test_entrypoints tests/test_language_array tests/test_lexer_safety tests/test_language_runtime tests/test_parser_array \
